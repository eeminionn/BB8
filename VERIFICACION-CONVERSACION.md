...
- Aplicación macOS compilada; inicio automático del proceso Python y estado `ready` verificados sin iniciar manualmente el servicio. La primera carga tarda más que una intervención posterior.

Los informes detallados están en `Logs/` (archivos locales, excluidos de Git). Esta batería pequeña comprueba funcionamiento, no demuestra precisión general de clasificación emocional ni sustituye la evaluación con participantes. El audio sintético comprueba STT y clasificación; el micrófono físico requiere la prueba del usuario y su permiso de macOS.
